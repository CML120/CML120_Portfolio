...
const AboutMe = () => {
  // State variables for toggling profile pictures and pulsing <h2>
  const [showProfilePicture2, setShowProfilePicture2] = useState(false);
  const [showH2Pulse, setShowH2Pulse] = useState(false);

  // Function to toggle profile picture
  const toggleProfilePicture = () => {
    setShowProfilePicture2(!showProfilePicture2);
  };

  // Function to toggle pulsing <h2>
  const toggleH2Pulse = () => {
    setShowH2Pulse(!showH2Pulse);
  };

  // Inline style for the profile picture
  const imageStyle = {
    float: 'right',
  ...
  return (
    <section>
      {/* <h2> with optional pulsing effect */}
      <h2 className={showH2Pulse ? 'h2-pulse' : ''}>About Me</h2>

      {/* profile picture that toggles on hover */}
      <img
